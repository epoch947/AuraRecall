...
export default function ResonancePage() {
  ...
  const [echoes, setEchoes]       = useState<PoolEcho[]>([])
  ...
  useEffect(() => {
    fetch('/api/resonance')
      .then((r) => r.json())
      .then((data) => { setEchoes(data.echoes ?? []); setLoading(false) })
  ...
  return (
    <div className="min-h-screen bg-oatmeal">
      {/* Header */}
      <div className="sticky top-0 z-20 bg-oatmeal/90 backdrop-blur-sm border-b border-sage/15
      ...
      <div className="max-w-2xl mx-auto px-8 py-16 flex flex-col gap-8">
        {loading && (
          <motion.p
            ...
            className="font-serif text-sm text-charcoal/40 italic text-center pt-24"
          >
            Listening to the pool…
          </motion.p>
        )}
              ...
              <div className="flex-1 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {echo.authorId && !isOwn && (
                    <button
                      onClick={() =>
                        setWhisper(isWhispering ? null : { echoId: echo.id, content: '', status: 'idle' })
                      }
                ...
                <AnimatePresence>
                  {isWhispering && (
                    ...
                    >
                      {whisper.status === 'sent' ? (
                        <p className="font-mono text-[10px] text-charcoal/40 tracking-[0.25em]
                                      italic py-2">
