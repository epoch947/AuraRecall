...
interface ConversationSummary {
  ...
  messages:    MessageRecord[] // latest message only
}

interface ConversationDetail extends Omit<ConversationSummary, 'messages'> {
...
function ConversationItem({
  ...
  const isOwn   = latest?.senderId === userId
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-1">
        <p className={`font-serif text-sm leading-snug truncate
                       ${pending ? 'text-oatmeal' : 'text-oatmeal/50'}`}>
          {latest?.content ?? '—'}
        </p>
        <p className="font-mono text-[9px] text-oatmeal/25 tracking-[0.25em] uppercase truncate">
          {conv.echo.weather} · {new Date(conv.updatedAt).toLocaleDateString('en-GB', {
            day: 'numeric', month: 'short',
          })}
        </p>
      </div>
      {pending && (
        <div className="w-1.5 h-1.5 rounded-full bg-oatmeal/40 flex-shrink-0 mt-2" />
      )}
    </motion.button>
  )
}
      ...
      <div className="flex-1 overflow-y-auto px-8 py-10 flex flex-col gap-8">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              <p className={`font-serif text-base leading-relaxed max-w-lg
  ...
  return (
    <AnimatePresence mode="wait">
      {activeConv ? (
        ...
        >
          {/* Header */}
          <div className="sticky top-0 z-20 bg-charcoal/90 backdrop-blur-sm border-b border-oatmeal/8
